...
import { ArrowLeft, ChevronRight, Clipboard, Compass, FileText, LoaderCircle, Search, Sparkles } from 'lucide-react';
import WorkflowSection from './WorkflowSection';

function WebAgentDeckProbeContent({ topLevelProbe, selectedProbe, onBack, probeData }) {
  ...
  const handleSubmit = () => {
    ...
    const steps = probeData.pipelineSteps ?? [];
    setVisibleStepCount(steps.length > 0 ? 1 : 0);
    ...
    <div className="flex min-h-[calc(100vh-6rem)] flex-col gap-8">
      <div className="space-y-5">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          >
            <p className="text-xs font-semibold uppercase tracking-[0.26em] text-slate-500">Prompt to deck</p>
            <p className="mt-2 text-sm leading-6 text-slate-700">
              The agent researches web sources, scans documents, and returns a generated slide deck instead of a video demo.
            </p>
          </div>
        </div>
      </div>
              ...
              {examples.map((example) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-3 line-clamp-6 whitespace-pre-wrap text-sm leading-6 text-slate-600">{example.prompt}</p>
                  </div>
                );
              })}
            </div>
              ...
              {outputState === 'loading' ? (
                <div className="flex min-h-[520px] flex-col justify-center">
                  <div className="flex flex-col items-center text-center">
                    ...
                    <p className="mt-4 text-lg font-semibold text-slate-900">{probeData.pipelineLabel}</p>
                    <p className="mt-2 max-w-lg text-sm leading-7 text-slate-600">
                      The agent is browsing web pages, opening documents, and organizing source material into a slide deck.
                    </p>
                  </div>
                  <div className="mt-8 space-y-3">
                    {(probeData.pipelineSteps ?? []).slice(0, visibleStepCount).map((step, index) => (
                      <div key={step} className="flex items-start gap-3 rounded-[18px] border border-slate-200 bg-white px-4 py-4">
                        <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-slate-700">
                          {index < 2 ? <Search className="h-4 w-4" /> : <FileText className="h-4 w-4" />}
                        </div>
                ...
                <div className="h-[520px] overflow-hidden rounded-[22px] border border-slate-200 bg-white">
                  <iframe
                    src={probeData.pdfPath}
                    title={`${selectedProbe.title} generated slide deck`}
                ...
                <div className="flex min-h-[520px] flex-col items-center justify-center text-center">
                  ...
                  <p className="mt-4 max-w-md text-sm leading-7 text-slate-600">
                    Press {probeData.submitLabel} to simulate the web research pipeline and open the generated slide deck.
                  </p>
                </div>
              )}
            </div>
...
export default function WebAgentDeckProbeLayout(props) {
  return <WebAgentDeckProbeContent key={props.selectedProbe.id} {...props} />;
}
